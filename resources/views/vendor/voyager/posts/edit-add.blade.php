                ...
                <div class="col-md-12">
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            ...
                            <textarea id="body" name="body" hidden>{{ $dataTypeContent->body ?? '' }}</textarea>
                        </div>
                    </div><!-- .panel -->
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @php
                                $dataTypeRows = $dataType->{($edit ? 'editRows' : 'addRows' )};
                                $exclude = ['title','id', 'body', 'excerpt', 'slug', 'status', 'category_id', 'author_id', 'featured', 'image','relatedtours','relatedtreks','relatedreads', 'metadisc', 'metakey', 'metatitle'];
                            @endphp
                            ...
                            <div class="form-group">
                                ...
                                @php
                                $trips = \App\Trip::get(['title','id']);
                                @endphp

                                        <label for="related">Related Tours</label>
                                        @include('voyager::multilingual.input-hidden', [
                                            '_field_name'  => 'relatedtours',
                                            '_field_trans' => get_field_translations($dataTypeContent, 'relatedtours')
                                        ])
                                     @php
$dataTypeContent->relatedtours = json_decode($dataTypeContent->relatedtours)
@endphp
<select class="form-control select2" name="relatedtours[]" multiple>

        @foreach($trips as $key => $label)
                <?php $selected = ''; ?>
            @if(is_array($dataTypeContent->relatedtours) && in_array($label['id'], $dataTypeContent->relatedtours))
                <?php $selected = 'selected="selected"'; ?>
            @elseif(!is_null(old($row->field)) && in_array($label['slug'], old($row->field)))
                <?php $selected = 'selected="selected"'; ?>
            @endif
...
<div class="form-group">
                                            ...
                                            <label for="related">Related Treks</label>
                                            @include('voyager::multilingual.input-hidden', [
                                                '_field_name'  => 'relatedtreks',
                                                '_field_trans' => get_field_translations($dataTypeContent, 'relatedtreks')
                                            ])
                                         @php
    $dataTypeContent->relatedtreks = json_decode($dataTypeContent->relatedtreks)
    @endphp
    <select class="form-control select2" name="relatedtreks[]" multiple>

            @foreach($trips as $key => $label)
                    <?php $selected = ''; ?>
                @if(is_array($dataTypeContent->relatedtreks) && in_array($label['id'], $dataTypeContent->relatedtreks))
                    <?php $selected = 'selected="selected"'; ?>
                @elseif(!is_null(old($row->field)) && in_array($label['slug'], old($row->field)))
                    <?php $selected = 'selected="selected"'; ?>
                @endif
    ...
    <div class="form-group">
            <label for="relatedreads">Related Reads</label>
                @include('voyager::multilingual.input-hidden', [
                    '_field_name'  => 'relatedreads',
                    '_field_trans' => get_field_translations($dataTypeContent, 'relatedreads')
                ])
                @php
        $dataTypeContent->relatedreads = json_decode($dataTypeContent->relatedreads)
        @endphp
        <select class="form-control select2" name="relatedreads[]" multiple>

                @foreach($posts as $key => $label)
                        <?php $selected = ''; ?>
                    @if(is_array($dataTypeContent->relatedreads) && in_array($label['id'], $dataTypeContent->relatedreads))
                        <?php $selected = 'selected="selected"'; ?>
                    @elseif(!is_null(old($row->field)) && in_array($label['slug'], old($row->field)))
                        <?php $selected = 'selected="selected"'; ?>
                    @endif
                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-bordered panel-primary">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="icon wb-image"></i> {{ __('voyager::post.image') }} (1366x768)</h3>
                            <div class="panel-actions">
                                <a class="panel-action voyager-angle-down" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                    ...
                    <div class="panel panel-bordered panel-info">
                        ...
                        <div class="panel-body">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="metadisc">{{ $dataTypeContent->metadisc ?? '' }}</textarea>
                            </div>
                            <div class="form-group">
                                <label for="meta_keywords">{{ __('voyager::post.meta_keywords') }}(Optional)</label>
                                @include('voyager::multilingual.input-hidden', [
                                    '_field_name'  => 'meta_keywords',
                                    '_field_trans' => get_field_translations($dataTypeContent, 'meta_keywords')
                                ])
                                <textarea class="form-control" name="metakey">{{ $dataTypeContent->metakey ?? '' }}</textarea>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="metatitle" placeholder="SEO Title" value="{{ $dataTypeContent->metatitle ?? '' }}">
                            </div>
...
@section('javascript')
    ...
    {{-- <script src="/js/ckeditor.js"></script> --}}
     ...
     window.addEventListener('DOMContentLoaded', () => {
         Laraberg.init('body', { height: '600px', laravelFilemanager: true, sidebar: true })
     })
     </script> 
@stop
