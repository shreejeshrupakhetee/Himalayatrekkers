...
@section('metatags')
    ...
    <title>Trekking in Nepal | Himalayan Trekkers</title>
    <style>
        #pageDestinations {
            width: 100%;
        }
        ...
        <main id="pageDestinations" class="pb-10 md:pb-20">
            <div class="secListing">
                <ul class="grid effect-2" id="grid">
                    @foreach ($regions as $activity)
                        <li>
                            <a href="{{ route('singleregion.index', ['slug' => $activity->slug]) }}">
                                <div class="img__holder">
                                    <img src="{{ Voyager::image($activity->image) }}" alt="{{ $activity->title }}" />
                                </div>
                                <div class="d_hold">
